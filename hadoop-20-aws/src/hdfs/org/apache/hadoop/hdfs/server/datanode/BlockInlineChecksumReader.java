...
public class BlockInlineChecksumReader extends DatanodeBlockReader {
  ...
  /**
   * return the blockDataFileReader
   */
  public BlockDataFile.Reader getBlockDataFileReader() {
    return blockDataFileReader;
  }

  /**
   */
  public int readDRCStripFully(byte buf[], int off, int len,
      long offset, byte checkbuf[] , int checkoff, 
      int bytesPerChecksum, int checksumSize, boolean throwOnEof) 
      throws IOException {
    return blockDataFileReader.readDRCStripFully(buf, off, len, offset, 
        checkbuf, checkoff, bytesPerChecksum, checksumSize, true);
  }

  /**
   */
  public void verifyDRCStripChecksum(int drcStripDataSize, int chunkNum,
      int bytesPerChecksum, int checksumSize, byte[] buf, int realOffset,
      byte[] checksumbuf) 
      throws IOException {
    int dataOff = 0;
    int checksumOff = 0;
    int remain = drcStripDataSize;
    for (int i = 0; i < chunkNum; i++) {
      ...
      if(!checksum.compare(checksumbuf, checksumOff)) {
        throw new ChecksumException("Checksum failed at " 
        + (realOffset + drcStripDataSize - remain), drcStripDataSize);
      }
  ...
  /**
   */
  public void checkBlockChanged(int checksumSize, int realEndOffset, 
      int bytesPerChecksum, byte[] buf, int drcStripDataSize) 
      throws IOException{
    if ((checksumSize != 0) && (realEndOffset % bytesPerChecksum != 0)
        && memoizedBlock.hasBlockChanged(realEndOffset)) {
      ChecksumUtil.updateChunkChecksum(buf, drcStripDataSize, 0,
          drcStripDataSize, checksum);
    }
