...
public class DFSInputStream extends FSInputStream {
  ...
  private long nodeEndLength=0;
  private long streamEnd=0;
  ...
  DFSInputStream(DFSClient dfsClient, String src, int buffersize, boolean verifyChecksum,
                 boolean clearOsBuffer, ReadOptions options, String code,
				 String jsonStr, int nodeId, int[] corruptArray, long nodeOutputLen) throws IOException {
    this(dfsClient, src, buffersize, verifyChecksum,
                   clearOsBuffer, options);
    this.code = code;
	this.jsonstr = jsonStr;
	this.nodeid = nodeId;
	this.corruptarray = corruptArray;
	this.nodeEndLength = nodeOutputLen;
  }
...
//	DataNode.LOG.info("DFSInputStream.init.buffersize = " + buffersize);
    this.clearOsBuffer = clearOsBuffer;
    prefetchSize = dfsClient.conf.getLong("dfs.read.prefetch.size",
    		10 * dfsClient.defaultBlockSize);//default 10 block size
  ...
  private LocatedBlock getBlockAt(long offset, boolean updatePosition,
    ...
    if (updatePosition) {
	  ...
	  if(this.code != null && this.code.equals("drc")) {
	    this.streamEnd = blk.getStartOffset()+this.nodeEndLength-1;
	  } else {
	    this.streamEnd = this.blockEnd;
	  }
    ...
    while (true) {
      ...
      try {
        ...
        if (dfsClient.shortCircuitLocalReads && isChosenDatanodeLocal) {
          DataNode.LOG.info("DFSInputStream.blockSeekTo want to get alocal blockreader");
    ...
    if (pos < getFileLength() || (pos == getFileLength() && len == 0)) {
      ...
      while (retries > 0) {
        try {
          ...
          // you want to update.
          //
//          DataNode.LOG.info("DFSInputStream.read3 pos = " + pos);
//          DataNode.LOG.info("DFSInputStream.read3 blockEnd = " + blockEnd);
    ...
    try {
      ...
      if (reader == null) {
        ...
        reader = getBlockReader(dfsClient.getDataTransferProtocolVersion(),
            ...
            cliData, options);
//        System.out.println("getBlockReader: targetAddr "+targetAddr);
  ...
  protected BlockReader getBlockReader(int protocolVersion,
    ...
    if (protocolVersion < DataTransferProtocol.READ_REUSE_CONNECTION_VERSION ||
      ...
      //deal with the drc node blockreader
      BlockReader reader = null;
      if(this.code != null) {
        reader = 
            BlockReader.newBlockReader(protocolVersion, namespaceId, sock, src,
                ...
                minReadSpeedBps, false, cliData, options, this.code,
				this.jsonstr, nodeid, corruptarray);
      } else {
        reader = 
            BlockReader.newBlockReader(protocolVersion, namespaceId, sock, src, 
                blockId, generationStamp, startOffset, len, buffersize, 
        ...
        if(this.code != null) {
          reader = 
              BlockReader.newBlockReader(protocolVersion, namespaceId, sock,src,
                  blockId, generationStamp, startOffset, len, buffersize,
                  verifyChecksum, clientName, bytesToCheckReadSpeed,
                  minReadSpeedBps, true, cliData, options, this.code,
				  this.jsonstr, nodeid, corruptarray);
        } else {
          reader = 
              BlockReader.newBlockReader(protocolVersion, namespaceId, sock, src, 
