...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  public int getnamespaceId() {
    return this.namenode.getNamespaceID();
  }
  ...
  public String getDataNodeByRackName(String rack) {
    return namenode.getDataNodeByRackName(rack);
  }

  public String[] getDatanodesByRack(String rack){
    return namenode.getDatanodesByRack(rack);
  }

  public String[] getRacks(){
    return namenode.getRacks();
  }
  ...
  private static LocatedBlocks callGetBlockLocations(
    ...
    try {
      if (supportMetaInfo) {
//        LOG.info("DFSClient.callGetBLockLocations.supportMetaInfo!!!");
  ...
  DFSInputStream open(String src, int buffersize, boolean verifyChecksum,
    ...
    if (blockLocationRenewal != null) {
      System.out.println("DFSClient.open6.blockLocationRenewal.add");
      blockLocationRenewal.add(stream);
    }
    return stream;
  }

  GroupInputStream open(int groupId, int groupSize, int buffersize,
      Map<Integer, LocatedBlockWithFileName> groupBlocks, int[] idxArray,
      int[] corruptArray, int outputUnitNum, boolean verifyChecksum, 
      boolean clearOsBuffer, ReadOptions options, String code, int toRead,
      String jsonStr) 
      throws IOException {
    checkOpen();
    GroupInputStream stream = new GroupInputStream(this, groupSize, buffersize,
        groupBlocks, idxArray, corruptArray, outputUnitNum, verifyChecksum, 
        clearOsBuffer, options, code, groupId, toRead, jsonStr);
    return stream;
  }

  DFSInputStream open(String src, int buffersize, boolean verifyChecksum,
      FileSystem.Statistics stats, boolean clearOsBuffer,
      ReadOptions options, String code, String jsonStr, int nodeId,
	  int[] corruptArray, long nodeOutputLen) throws IOException {

    checkOpen();

    incFileReadToStats();
    DFSInputStream stream = new DFSInputStream(this, src, buffersize,
        verifyChecksum, clearOsBuffer, options, code, jsonStr, nodeId,
		corruptArray, nodeOutputLen);
    if (blockLocationRenewal != null) {
      blockLocationRenewal.add(stream);
    }
    return stream;
...
//  System.out.println("DFSClient.create9");
  return create(src, permission, overwrite, createParent, replication, blockSize,
...
//    System.out.println("DFSClient.create13"); 
    if (permission == null) {
...
//      System.out.println("DFSClient.createDFSOutputStream");
  ...
  public FileStatus[] listPaths(String src) throws IOException {
    ...
    try {
      if (namenodeProtocolProxy == null) {
//        System.out.println("DFSClient.listPaths.namenodeProtocolProxy==null");
...
//      System.out.println("DFSClient.methodBasedListPath.getPartialListing");
      return iterativeListing(src);
    } else if (namenodeProtocolProxy.isMethodSupported(
        "getHdfsListing", String.class)) {
//      System.out.println("DFSClient.methodBasedListPath.getHdfsListing");
      HdfsFileStatus[] hdfsStats = namenode.getHdfsListing(src);
      if (hdfsStats == null) {
        return null;
      }
...
//      System.out.println("DFSClient.methodBasedListPath.else");
      return namenode.getListing(src);
    }
  }
...
//        LOG.info("DFSClient.getFileInfo.cache!=null");
        fileStatus = fileStatusCache.get(src);
        if (fileStatus != FileStatusCache.nullFileStatus) {
//          LOG.info("DFSClient.getFileInfo.status!=null");
          return fileStatus;
        }
//        LOG.info("DFSClient.getFileInfo.status==null");
      }
//      LOG.info("DFSClient.getFileInfo.cache==null");
//      LOG.info("DFSClient.getFileInfo.protocol==null ? " + (namenodeProtocolProxy==null));
...
//      LOG.info("DFSClient.getFileInfo.status==null?" + (fileStatus == null));
...
//      LOG.info("DFSClient.methodBasedGetFileInfo.support");
      return HdfsFileStatus.toFileStatus(namenode.getHdfsFileInfo(src), src);
    } else {
//      LOG.info("DFSClient.methodBasedGetFileInfo.not support");
      return namenode.getFileInfo(src);
    }
  }
  ...
  public CorruptFileBlocks listCorruptFileBlocks(String path,
    ...
    throws IOException {
    if (namenodeProtocolProxy == null) {
//      LOG.info("DFSClient.namenodeProtocolProxy == null");
...
//    LOG.info("DFSClient.methodBasedListCorruptFileBlocks.support!!!");
    return namenode.listCorruptFileBlocks(path, cookie);
  }
