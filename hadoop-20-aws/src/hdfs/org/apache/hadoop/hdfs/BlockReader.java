...
//    LOG.info("BlockReader.read3 want to read from " + off + " to " + len);
  ...
  protected synchronized int readChunk(long pos, byte[] buf, int offset,
    ...
    /**
     * I don't think the statement after if is right, pos should denote chunkOffset
     */
  ...
  public static BlockReader newBlockReader( int dataTransferVersion,
                                     ...
                                     FSClientReadProfilingData cliData,
                                     ReadOptions options, String code,
									 String jsonStr, int nodeid,
									 int[] corruptarray)
                                     throws IOException {
    if(code.equals("drc")) {
      // create socket outputstream
      DataOutputStream out = new DataOutputStream(
        new BufferedOutputStream(NetUtils.getOutputStream(sock,HdfsConstants.WRITE_TIMEOUT)));

      // write the header. TODO: should be wrapped in a new class???
      //write the datatransferversion and opcode
      //which denotes that this is the node level step of the drc recover process
      ReadDRCBlockHeader readDRCBlockHeader = new ReadDRCBlockHeader(
          dataTransferVersion, namespaceId, blockId, genStamp, startOffset, len,
          clientName, reuseConnection, cliData != null, jsonStr,
		  nodeid, corruptarray);
      readDRCBlockHeader.setReadOptions(options);
      readDRCBlockHeader.writeVersionAndOpCode(out);
      readDRCBlockHeader.write(out);
    ...
    if (in.readShort() != DataTransferProtocol.OP_STATUS_SUCCESS) {
      throw new IOException("Got error in reponse to OP_DRC_NODE_LEVEL " +
