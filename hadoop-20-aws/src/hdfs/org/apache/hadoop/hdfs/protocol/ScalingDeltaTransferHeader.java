...
public class ScalingDeltaTransferHeader extends DataTransferHeader implements Writable {
  ...
  public ScalingDeltaTransferHeader(final VersionAndOpcode versionAndOp, Configuration conf) {
    super(versionAndOp);
    this.conf = conf;
  }

  public ScalingDeltaTransferHeader(int dataTransferVersion, final int namespaceId, int deltaTransferID,
    DistributedFileSystem fs, Map<Integer, LocatedBlockWithFileName> parityBlocks, byte[][] delta, String[] nodesName) {
    super(dataTransferVersion, DataTransferProtocol.OP_DELTA_TRANSFER_IN_SCALING);
