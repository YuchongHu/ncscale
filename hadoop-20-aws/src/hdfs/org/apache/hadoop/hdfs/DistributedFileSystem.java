...
public class DistributedFileSystem extends FileSystem {
  ...
  /**
   * some class get an object of DistributedFileSystem 
   * and thus can use DFSClient to request the topology
   * information of the whole cluster
   */
  public String getDataNodeByRackName(String rack) {
    return dfs.getDataNodeByRackName(rack);
  }

  public String[] getDatanodesByRack(String rack) {
    return dfs.getDatanodesByRack(rack);
  }
  ...
  public GroupInputStream open(int groupId, int groupSize, int bufferSize, 
      Map<Integer, LocatedBlockWithFileName> groupBlocks, int outputUnitNum, 
      String code, ReadOptions options, int toRead, int[] corruptArray,
      int[] idxArray, String jsonStr)
      throws IOException {
    return dfs.open(groupId, groupSize, bufferSize, groupBlocks, idxArray, corruptArray,
        outputUnitNum, verifyChecksum, clearOsBuffer, options, code, toRead, jsonStr);
  }

  public FSDataInputStream open(Path f, int bufferSize,
      String code, String jsonStr, int nodeId, int[] corruptArray,
	  long nodeOutputLen) throws IOException {
    return open(f, bufferSize, code, jsonStr, nodeId, 
	    corruptArray, nodeOutputLen,new ReadOptions());
  }

  public FSDataInputStream open(Path f, int bufferSize, String code,
      String jsonStr, int nodeId, int[] corruptArray, long nodeOutputLen,
	  ReadOptions options) throws IOException {
    return new DFSClient.DFSDataInputStream(
          dfs.open(getPathName(f), bufferSize, verifyChecksum, statistics,
                   clearOsBuffer, options, code, jsonStr, nodeId,
  ...
  public FSDataOutputStream create(Path f, FsPermission permission,
    ...
    Progressable progress) throws IOException {
      return create(f, permission, overwrite, bufferSize,
        replication,
        blockSize,
        getConf().getInt("io.bytes.per.checksum",FSConstants.DEFAULT_BYTES_PER_CHECKSUM), progress);
