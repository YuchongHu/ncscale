...
public class NetworkTopology {
  ...
  /**
   * Get the racks using the rackId.
   */
  public String getRack(int rackId) {
    netlock.readLock().lock();
    try {
      return getRacks().get(rackId);
    } finally {
      netlock.readLock().unlock();
    }
  }
  ...
  public Map<String, List<String>> networkToMap(){
    ...
    int numOfLeaves = getNumOfLeaves();
    for(int i=0; i<numOfLeaves; i++) {
      String node = NodeBase.getPath(clusterMap.getLeaf(i,null));
      LOG.info("node" + i + " = " + node);
      String[] contents = node.split("/");
      String rack = "/" + contents[1];
      String addr = contents[2];
      List<String> thisrack = map.get(rack);
      if(thisrack == null) {
        thisrack = new LinkedList<String>();
        thisrack.add(addr);
        map.put(rack, thisrack);
      } else {
        thisrack.add(addr);
      }
    }
