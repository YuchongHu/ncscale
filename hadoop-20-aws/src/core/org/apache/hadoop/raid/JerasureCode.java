...
public class JerasureCode extends ErasureCode {
  ...
  throws IOException {
    ...
    LOG.info("JerasureCode.encodeBulk3.stripeSize = " + stripeSize);
    LOG.info("JerasureCode.encodeBulk3.paritySize = " + paritySize);
  ...
  public List<Integer> locationsToReadForDecode(List<Integer> erasedLocations)
    ...
    int totalLen = stripeSize() + paritySize();
    int occuppiedParity=0;
    for(Integer i:erasedLocations){
      if(i>=0){
        occuppiedParity++;
      }
    }
    for(int i=0;i<paritySize();i++){
      if(erasedLocations.indexOf(i)==-1){
        locationsToRead.add(i);
      }
      if(locationsToRead.size()==occuppiedParity){
        break;
      }
    }
    for(int i=0;i<stripeSize();i++){
      if(erasedLocations.indexOf(i+paritySize())==-1){
        locationsToRead.add(i+paritySize());
      }
    ...
    if(locationsToRead.size()!=stripeSize()){
      throw new TooManyErasedLocations("Locations " + locationsToRead.toString());
    }
    /* Debug info begins */
