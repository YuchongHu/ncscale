...
public class ParityFilePair {
  ...
  public static ParityFilePair getParityFile(Codec codec, FileStatus srcStat,
    ...
    RaidNode.LOG.info("ParityFilePair.return null");

    return null; // NULL if no parity file
  }

  public static ParityFilePair getDRCParityFile(Codec codec, FileStatus srcStat,
      Configuration conf, boolean skipHarChecking) throws IOException {
    ...
    if (!codec.isDirRaid && !skipHarChecking) {
      ...
      if (fsDest.exists(HarPath)) {
        ...
        FileStatus inHar = FileStatusCache.get(fsHar, inHarPath);
        if (inHar != null) {
          ParityFilePair pfp = verifyDRCParity(srcStat, inHar, codec, conf, fsHar);
          if (pfp != null) {
            return pfp;
          }
        }
    ...
    try {
      FileStatus outHar = fsDest.getFileStatus(outPath);
      ParityFilePair pfp = verifyDRCParity(srcStat, outHar, codec, conf, fsDest);
      if (pfp != null) {
        return pfp;
      }
    } catch (java.io.FileNotFoundException e) {
  ...
  static ParityFilePair verifyDRCParity(FileStatus src, FileStatus parity,
    ...
    if (codec.isDirRaid) {
      ...
      lfs = RaidNode.listDRCDirectoryRaidFileStatus(conf, srcFs, 
          src.getPath());
      if (lfs == null) {
        return null;
      }
  ...
  static ParityFilePair verifyParity(FileStatus src, FileStatus parity,
    ...
    if (codec.isDirRaid) {
      FileSystem srcFs = src.getPath().getFileSystem(conf);
      RaidNode.LOG.info("verifyParity.srcpath = " + src.getPath().toUri());
      RaidNode.LOG.info("verifyParity.srcFs == null ? " + (srcFs == null));
      lfs = RaidNode.listDirectoryRaidFileStatus(conf, srcFs, 
          src.getPath());
      if (lfs == null) {
        RaidNode.LOG.info("ParityFilePair.verifyParity lfs == null");
        return null;
      }
