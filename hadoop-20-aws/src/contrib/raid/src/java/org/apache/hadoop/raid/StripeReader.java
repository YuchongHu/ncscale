...
public abstract class StripeReader {
  ...
  public InputStream[] buildInputs(
    ...
    do {
      ...
      for (int i = 0; i < inputs.length; i++) {
        boolean isErased = (erasedLocations.indexOf(i) != -1);
        boolean shouldRead = (locationsToRead.indexOf(i) != -1);
        LOG.info("StripeReader.buildInputs." + i + "isErased ? " + isErased);
        LOG.info("StripeReader.buildInputs." + i + "shouldread ? " + shouldRead);
        try {
          InputStream stm = null;
          if (isErased || !shouldRead) {
            if (isErased) {
    ...
    assert(locationsToNotRead.size() == codec.parityLength);
    LOG.info("StripeReader is returning inputs");
    return inputs;
  }
  ...
  public static LocationPair getDRCBlockLocation(Codec codec, FileSystem srcFs,
      Path srcFile, int blockIdxInFile, Configuration conf, 
    ...
    if (codec.isDirRaid) {
      ...
      if (lfs == null) {
        lfs = RaidNode.listDRCDirectoryRaidFileStatus(conf, srcFs, parentPath);
      }
      if (lfs == null) {
