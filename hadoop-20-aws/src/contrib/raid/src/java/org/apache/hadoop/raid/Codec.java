...
public class Codec implements Serializable {
  ...
  public final int drcType;
  //scale-up num
  public final int newNode;
  //The number of nodes in HDFS for MBR
...
//      Configuration.addDefaultResource("raid-site.xml");
  ...
  public static void initializeCodecs(Configuration conf) throws IOException {
    try {
      ...
      if (source == null) {
//        LOG.info("Codec.initializeCodecs.source == null");
      ...
      LOG.info("Codec.initializeCodecs.source = " + source);
      ...
      for (int i = 0; i < jsonArray.length(); ++i) {
        ...
        LOG.info("Codec.initializeCodecs.codec.id = " + codec.id);
        LOG.info("Codec.initializeCodecs.jsonStr = " + jsonArray.getJSONObject(i).toString());
      }
  ...
  private Codec(JSONObject json) throws JSONException {
    ...
    if (this.id.equals("drc")) {
      this.groupNum = json.getInt("group_num");
      this.groupSize = json.getInt("group_size");
      this.drcType = json.getInt("drc_type");
      this.newNode = json.getInt("new_node");
      this.nodeNum = json.getInt("node_num");
    } else {
      this.groupNum = 0;
      this.groupSize = 0;
      this.drcType = 0;
...
//      LOG.info("Codec.createErasureCode jsonStr = " + this.jsonStr);
//      LOG.info("Codec.createErasureCode id = " + this.id);
//      LOG.info("Codec.createErasureCode parityLength = " + this.parityLength);
//      LOG.info("Codec.createErasureCode stripeLength = " + this.stripeLength);
//      LOG.info("Codec.createErasureCode erasureCodeClass = " + this.erasureCodeClass);
//      LOG.info("Codec.createErasureCode parityDirectory = " + this.parityDirectory);
//      LOG.info("Codec.createErasureCode isDirRaid = " + this.isDirRaid);
    } catch (ClassNotFoundException e) {
      throw new RuntimeException(e);
    }
    ErasureCode code = (ErasureCode) ReflectionUtils.newInstance(erasureCode,
        conf);
    code.init(this.stripeLength, this.parityLength, this.jsonStr);
    return code;
  }
