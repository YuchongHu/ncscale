 ...
 */
public class BlockPlacementPolicyJerasure extends BlockPlacementPolicyRaid {
  ...
  BlockPlacementPolicyJerasure(Configuration conf,
                         FSClusterStats stats,
                               NetworkTopology clusterMap) {
    initialize(conf, stats, clusterMap, null, null, null);
  }

  BlockPlacementPolicyJerasure() {
  }
  ...
  private DatanodeDescriptor[] chooseTargetF4(
    ...
    try {
      ...
      switch (info.type) {
          ...
          if(!indexToNodeCreated){
            for(String rack : this.clusterMap.getRacks()){
              for(Node node : this.clusterMap.getDatanodesInRack(rack)) {
                ...
                LOG.info(" debug msg: indexToNode[" + index + "]: " + indexToNode[index]);
                index ++;
              }
            }
            for(int i=0;i<20;i++){
              for(int j=i+1;j<20;j++){
                if(indexToNode[i].toString().compareTo(indexToNode[j].toString())>0){
                  Node tmpNode=indexToNode[i];
                  indexToNode[i]=indexToNode[j];
                  indexToNode[j]=tmpNode;
                }
              }
            }
            indexToNodeCreated = true;
          }
          stripeIndex = blockIndex/dataLen%20;
          blkIndexInStripe = blockIndex%dataLen;
          rackId = (stripeIndex%totalLen + blkIndexInStripe)%totalLen;
          nodeIdxInRack = stripeIndex/totalLen;
          nodeIdx = rackId*4 + nodeIdxInRack;
          LOG.info("debug info: blockIndex: " + blockIndex + " nodeIndex: " + nodeIdx);
          retVal.add((DatanodeDescriptor)indexToNode[nodeIdx]);
        ...
        case PARITY:
          fNSplits = fileName.split("_");
          stripeIndex = Integer.parseInt(fNSplits[fNSplits.length-1])%20;
          blkIndexInStripe = blockIndex%parityLen + dataLen;
          rackId = (stripeIndex%totalLen + blkIndexInStripe)%totalLen;
          nodeIdxInRack = stripeIndex/totalLen;
          nodeIdx = rackId*4 + nodeIdxInRack;
          //LOG.info("debug msg: parity file stripeIndex: " + stripeIndex);
          LOG.info("debug info: parity stripeIndex: " + stripeIndex + 
              "blockIndex: " + blockIndex + " nodeIndex: " + nodeIdx + indexToNode[nodeIdx]);
          retVal.add((DatanodeDescriptor)indexToNode[nodeIdx]);
          return retVal.toArray(new DatanodeDescriptor[retVal.size()]);
          //return super.chooseTarget(
          //  fileName, numOfReplicas, writer, chosenNodes, exclNodes, blocksize);
