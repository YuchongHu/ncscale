...
public class Decoder {
  ...
  DecoderInputStream generateAlternateStream(FileSystem srcFs, Path srcFile,
	...
	DecoderInputStream decoderInputStream = new DecoderInputStream(
		...
		lostBlock, si, recoverFromStripeInfo, code);
  ...
  long fixErasedBlockImpl(FileSystem srcFs, Path srcFile, FileSystem parityFs,
    ...
    LOG.info("Decoder.fixErasedBlockImpl.error Offset = " + errorOffset);
    LOG.info("Decoder.fixErasedBlockImpl.blockIdx = " + blockIdx);
    LocationPair lp = null;
    int erasedLocationToFix;
    
    if (recoverFromStripeStore) {
      LOG.info("Decoder.fixErasedBlockImpl.recoverFromStripeStore");
      erasedLocationToFix = si.getBlockIdxInStripe(lostBlock);
    } else if (fixSource) {
      LOG.info("Decoder.fixErasedBlockImpl.fixSource");
      lp = StripeReader.getBlockLocation(codec, srcFs,
          srcFile, blockIdx, conf);
      LOG.info("Decoder.fixErasedBlockImpl.stripeIdx = " + lp.getStripeIdx());
      LOG.info("Decoder.fixErasedBlockImpl.BlockIdxInStripe = " + lp.getBlockIdxInStripe());
      erasedLocationToFix = codec.parityLength + lp.getBlockIdxInStripe(); 
    } else {
      LOG.info("Decoder.fixErasedBlockImpl.else");
    ...
    try {
      ...
      for (written = 0; written < limit; ) {
        try {
          if (parallelReader == null) {
            LOG.info("Decoder.fixErasedBlockImpl.parallelReader==null");
            ...
            if (recoverFromStripeStore) {
              ...
              LOG.info("Decoder.fixErasedBlockImpl.not recoverfromstripestore");
            ...
            LOG.info("Decoder.fixErasedBlockImpl.going to create parallelastreamreader");
            LOG.info("Decoder.fixErasedBlockImpl.bufSize = " + bufSize);
            LOG.info("Decoder.fixErasedBlockImpl.limit = " + limit);
            LOG.info("Decoder.fixErasedBlockImpl.boundedBufferCapacity = " + boundedBufferCapacity);
            LOG.info("Decoder.fixErasedBlockImpl.blockSize = " + blockSize);
          ...
          LOG.info("Decoder.fixErasedBlockImpl.going to readFromInputs");
          ParallelStreamReader.ReadResult readResult = readFromInputs(
            erasedLocations, limit, reporter, parallelReader);         
         
          LOG.info("Decoder.fixErasedBlockImpl.readFromInputs ends"); 
          ...
          LOG.info("Decoder.fixErasedBlockImpl.going to dodecoding");
		  ...
		  this.calculatetime += endCalculate - startCalculate;
		  this.totaltime += endCalculate - startCalculate;
		  ...
		  this.writetime += endWrite - startWrite;
		  this.totaltime += endWrite - startWrite;
        } catch (IOException e) {
      ...
      this.readtime = parallelReader.readTime;
	  this.totaltime += this.readtime;
	  LOG.info("Decoder.fixErasedBlockImpl.readtime = " +
	      this.readtime);
	  LOG.info("Decoder.fixErasedBlockImpl.calculatetime = " +
	      this.calculatetime);
  ...
  public class DecoderInputStream extends InputStream {
	...
	private LocationPair drclp;
	private FileStatus drcSrcStat;
    private int errIdxInFile;
	private String drcJsonStr;
	private Map<Integer, LocatedBlockWithFileName> drcStripeBlocks = 
	    new HashMap<Integer, LocatedBlockWithFileName>();
	private int[] drcCorruptArray;
	private int lostIdx = -1;
	private DRCDecoder drcDecoder = null;
	private boolean useDrc = false;

	public DecoderInputStream(
	    final Progressable reporter, final long limit, 
		final long blockSize, final long errorOffset,
		final FileSystem srcFs, final Path srcFile,
		final FileSystem parityFs, final Path parityFile,
		Block lostBlock, StripeInfo si,
		boolean recoverFromStripeInfo, String code) {

      this(reporter, limit, blockSize, errorOffset, 
	      srcFs, srcFile, parityFs, parityFile, lostBlock, si, 
		  recoverFromStripeInfo);


	  //TODO: codec.jsonStr find stripeBlocks via lostBlock and
	  //filepath
	  // 1 
	  try {
        this.drcSrcStat = srcFs.getFileStatus(srcFile);
        this.errIdxInFile = (int)(errorOffset/blockSize);
	    this.drclp = StripeReader.getBlockLocation(codec, srcFs,
	        srcFile, errIdxInFile, conf);
	    this.drcJsonStr = codec.jsonStr;
	    System.out.println(this.drcJsonStr);
        
		this.drcCorruptArray = new int[codec.stripeLength +	codec.parityLength];

	    int startBlkNum = drclp.getStripeIdx() * codec.stripeLength;
		this.useDrc = true;
   
        int blkcount = 0;
		int nowcount = startBlkNum;
		int idxInStripe=0;
  	    for(FileStatus fsStat: drclp.getListFileStatus()) {
		  Path nowpath = fsStat.getPath();
		  DistributedFileSystem nowfs = getDFS(nowpath, conf);
          LocatedBlocksWithMetaInfo blocksInfo =
		      nowfs.getClient().namenode.
		      openAndFetchMetaInfo(nowpath.toUri().getPath(), 0, fsStat.getLen());
		  int num = blocksInfo.locatedBlockCount();
		  if(blkcount<=nowcount && nowcount<(blkcount+num)) {
		    for(int i=nowcount-blkcount; i<num && idxInStripe<codec.stripeLength; i++) {
			  LocatedBlock lb = blocksInfo.get(i);
              if(lb.getBlock().getBlockId() == lostBlock.getBlockId()) {
			    lostIdx = idxInStripe;
		      }
			  if(lb.isCorrupt()) {
			    drcCorruptArray[idxInStripe] = 1;
			  } else {
			    drcCorruptArray[idxInStripe] = 0;
			  }
              this.drcStripeBlocks.put(idxInStripe++, 
			      new LocatedBlockWithFileName(lb.getBlock(),
				      lb.getLocations(), lb.getStartOffset(),
					  lb.isCorrupt(), nowpath.toUri().getPath()));
			  nowcount++;
			}
		  }
		  blkcount += num;
        ...
        nowcount = drclp.getStripeIdx() * codec.parityLength;
	    DistributedFileSystem pfs = getDFS(parityFile, conf);
		FileStatus pstat = pfs.getFileStatus(parityFile);
		LocatedBlocksWithMetaInfo blocksInfo = pfs.getClient().namenode.openAndFetchMetaInfo(
		    parityFile.toUri().getPath(), 0, pstat.getLen());
		int num = blocksInfo.locatedBlockCount();
		for(int i= nowcount; 
		    i<num && idxInStripe<(codec.stripeLength+codec.parityLength);
			i++) {
		  LocatedBlock lb = blocksInfo.get(i);
		  this.drcStripeBlocks.put(idxInStripe++,
              new LocatedBlockWithFileName(lb.getBlock(),
			      lb.getLocations(), lb.getStartOffset(),
				  lb.isCorrupt(), parityFile.toUri().getPath()));
		  nowcount++;
		}
	...
	public DistributedFileSystem getDFS(Path p, Configuration conf)
	    throws IOException {
	...
	private void init(boolean drc) throws IOException {
	  if(startTime == -1) {
	    startTime = System.currentTimeMillis();
      }
	  ...
	  if(null == drcDecoder) {
	    this.drcDecoder = new DRCDecoder(conf, codec.jsonStr, this.drcStripeBlocks,
		    this.drcCorruptArray, true, lostIdx);
		drcDecoder.recoverBlockToFile();
	  }
	  ...
	  if(null == buffer) {
	    bufferLen = Math.min(bufSize, limit - streamOffset);
		buffer = drcDecoder.getBufferForDegraded((int)bufferLen);
		constructedBytes += bufferLen;
		position = 0;
	  }
	  success = true;
	}

	private void checkBuffer(boolean drc)  throws IOException{
	  try {
	    if(!drc) {
	      checkBuffer();
        } else {
