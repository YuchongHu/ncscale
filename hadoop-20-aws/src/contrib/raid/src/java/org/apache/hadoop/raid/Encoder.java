...
public class Encoder {
  ...
  Encoder(Configuration conf, Codec codec) {
    ...
    LOG.info("Encoder.constructor.maxBufSize = " + maxBufSize);
//    this.bufSize = conf.getInt("raid.encoder.bufsize", maxBufSize);
    this.bufSize = conf.getInt("hdfs.raid.strip.size", maxBufSize);
    LOG.info("Encoder.constructor.bufSize = " + bufSize);
    ...
    if (codec.isDirRaid) {
      ...
      LOG.info("Encoder.constructor.isDirRaid.going to createStripeStore");
  ...
  public boolean encodeTmpParityFile(Configuration jobConf, StripeReader sReader,
    ...
    String[] partialTmpSplits = partialTmpParity.getName().split("/");
    Path parityTmp = new Path(tmpDir, parityFile.getName() + rand.nextLong() + "-" + partialTmpSplits[partialTmpSplits.length-1]);
    FSDataOutputStream out = parityFs.create(
                               ...
                               tmpRepl,
                               blockSize);
//    LOG.info("66666666666666"+partialTmpParity);
//    LOG.info("77777777777777"+parityFile);
//    LOG.info("88888888888888"+parityTmp);
    try {
      CRC32[] crcOuts = null;
      ...
      if (checksumStore != null) {
        LOG.info("Encoder.encodeTmpParityFile.checksumStore != null");
  ...
  public boolean encodeFile(
    ...
    if (ec.srcStripes == null && stripeStore != null) {
      LOG.info("Encoder.encodeFile.goingto getSrcStripes");
      InjectionHandler.processEventIO(
  ...
  private void encodeFileToStream(StripeReader sReader,
    ...
    try {
      ...
      while (sReader.hasNext()) {
        ...
        LOG.info("Encoder.encodeFileToStream. currentStripeIdx = " + currentStripeIdx);
        stripeInputInfo = sReader.getNextStripeInputs();
        // The offset of first temporary output stream
        long encodeStartOffset = out.getPos();
        LOG.info("Encoder.encoderFileToStream. encodeStartOffset = " + encodeStartOffset);
        int retry = 3;
        do {
          redo = false;
          retry --;
