...
class BlockMover {
  ...
  /**
   * explicitly choose a node on target rack
   * @throws IOException
   *
   */
  public DatanodeInfo chooseTargetNodeOnSameRack(Set<DatanodeInfo> includedNodes) 
      throws IOException {
    DatanodeInfo target = cluster.getNodeOnSameRack(includedNodes);
    if (target == null) {
      throw new IOException ("Error choose datanode");
    }
    return target;
   ...
   * Choose three nodes
   *
   * Three nodes reside on different racks
   * Each rack where the node resides on has
   * not less than numNodes nodes
   * This function is used by DRC - (6z, 3z, 3)
   * @throws IOException
   *
   */
  public List<DatanodeInfo> getThreeNodes(int numNodes) 
      throws IOException {
    List<DatanodeInfo> ret = cluster.getThreeNodes(numNodes);
    if (ret == null) {
      throw new IOException ("Error choose datanode");
    }
    return ret;
    ...
    public void run() {
      ...
      try {
        ...
        if (target.getName().equals(source.getName())) {
            LOG.info("BlockMover: the target equals to the source in run().");
            return;
        }
        sock = new Socket();
        sock.connect(NetUtils.createSocketAddr(
            target.getName()), HdfsConstants.READ_TIMEOUT);
        LOG.info("FSConstants.BUFFER_SIZE: " + FSConstants.BUFFER_SIZE);
        LOG.info("HdfsConstants.READ_TIMEOUT: " + HdfsConstants.READ_TIMEOUT);
        ...
        receiveResponse(in);
        metrics.blockMove.inc();
        LOG.info(threadName + "threadID: " + Thread.currentThread().getId());
        ...
        try {
          ...
          LOG.warn(threadName + "threadID: " + Thread.currentThread().getId());
  ...
  class ClusterInfo implements Runnable {
    ...
    @Override
    public void run() {
//      LOG.info("placementmonitor.blockmover.clusterinfo start");
        ...
        try {
          ...
          if (now - lastUpdate > UPDATE_PERIOD) {
            ...
            synchronized (this) {
              ...
              LOG.info("clusterinfo.add topology");
              for (DatanodeInfo n : liveNodes) {
                topology.add(n);
              }
            }
          }
//          LOG.info("placementmonitor.blockmover.clusterinfo.going to sleep");
     ...
     *
     * Three nodes reside on different racks
     * Each rack where each node resides on has
     * not less than numNodes nodes
     * This function is used by DRC - (6z, 3z, 3)
     *
     */
    public synchronized List<DatanodeInfo> getThreeNodes(int numNodes) {
      if (liveNodes == null || liveNodes.length == 0) {
        return null;
      }
      List<DatanodeInfo> ret = new ArrayList<DatanodeInfo>();
      List<String> candRack = new ArrayList<String>();
      for (String curRack : topology.getAllRacks()) {
        if(topology.getDatanodesInRack(curRack).size() >= numNodes) {
          candRack.add(curRack);
        }
      }
      if (candRack.size() < 3) return null;
      int rackIdx = rand.nextInt(candRack.size());
      for (int i = 0; i < 3; i++) {
        rackIdx = (++rackIdx) % candRack.size();
        List<Node> candNodes = topology.getDatanodesInRack(candRack.get(rackIdx));
        int targetIdx = rand.nextInt(candNodes.size());
        ret.add((DatanodeInfo)candNodes.get(targetIdx));
      }   
      return ret;
    }
    ...
    /**
     *Choose a node on same rack
     */
    public synchronized DatanodeInfo getNodeOnSameRack(
                                      Set<DatanodeInfo> included) {
      if (liveNodes == null || liveNodes.length == 0) {
        return null;
      }
      
      DatanodeInfo[] includedNodes = new DatanodeInfo[included.size()];
      includedNodes = included.toArray(includedNodes);
      String sameRack = null;
      for (String curRack : topology.getAllRacks()) {
        if (curRack.contains(includedNodes[0].getParent().getName())) {
          sameRack = curRack;
          break;
        }
      }
      
      List<Node> candidateNodes = topology.getDatanodesInRack(sameRack);
      if (sameRack == null || candidateNodes.size() <= includedNodes.length) {
        return null;
      }
      int targetIdx = rand.nextInt(candidateNodes.size());
      boolean sameNode = true;
      while (sameNode) {
        targetIdx = (++targetIdx) % candidateNodes.size();
        int i = 0;
        for (; i < includedNodes.length; i++) {
          if(candidateNodes.get(targetIdx).getName().equals(includedNodes[i].getName())) {
            sameNode = true;
            break;
          }
        }
        if (i == includedNodes.length) sameNode = false;
      }
      
      return (DatanodeInfo)candidateNodes.get(targetIdx); 
    }
