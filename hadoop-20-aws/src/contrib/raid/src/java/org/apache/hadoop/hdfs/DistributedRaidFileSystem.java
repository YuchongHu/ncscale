...
public class DistributedRaidFileSystem extends FilterFileSystem {
  ...
  private static class ExtFSDataInputStream extends FSDataInputStream {
    ...
    private static class ExtFsInputStream extends FSInputStream {
	  ...
	  private boolean needRelayer;   //  add

      private long degradedstart;
      private long degradedstop;
      private long degradedtime;
      ...
      ExtFsInputStream(Configuration conf, DistributedRaidFileSystem lfs,
		...
		this.needRelayer = this.conf.getBoolean("raid.needrelayer", false);
      ...
      public synchronized int read(byte[] b, int offset, int len) 
        ...
        boolean before = useRecoveryStream;

        openCurrentStream();
        int limit = Math.min(blockAvailable(), len);
        int value;
        if (!before && useRecoveryStream) {
            degradedstart = System.currentTimeMillis();
        } else if (before && !useRecoveryStream) {
            degradedstop = System.currentTimeMillis();
            degradedtime = degradedstop - degradedstart;
            System.out.println("DRC degraded read use " + degradedtime + " ms.");
        }
        if (useRecoveryStream) {
          ...
          try{
		    ...
		    System.out.println("DistributedRaidFileSystem.read3.BlockMissingException");
            value = readViaCodec(b, offset, limit, blockAvailable(), e, this.needRelayer);  // add needRelayer
          ...
          if (useRecoveryStream) {
            ...
            try{
              ...
              value = readViaCodec(b, offset, limit, limit, e,
			      this.needRelayer); // add needRelayer
            } catch (ChecksumException e) {
              value = readViaCodec(b, offset, limit, limit, e,
			      this.needRelayer); // add needRelayer
            }            
      ...
      private int readViaCodec(byte[] b, int offset, int len,
          int streamLimit, IOException e, boolean needRelayer) 
          throws IOException{
        ...
        try {
          if (null == recoveryStream || 
            ...
            recoveryStream = getAlternateInputStream(e, currentOffset, 
                streamLimit, needRelayer);  // add needRelayer
          }
      ...
      private DecoderInputStream getAlternateInputStream(IOException curexp, 
            long offset, 
            final long readLimit, boolean needRelayer) 
            throws IOException{
        ...
        while (nextLocation < Codec.getCodecs().size()) {
          try {
			...
			if(codec.id.equals("drc")) {
			  recoveryStream = RaidNode.unRaidCorruptInputStream(innerConf, path,
			      codec, parityFilePairs.get(codec.id), lostBlock.getBlock(),
				  blockSize, offset, limit, false, codec.id);
		    } else {
              recoveryStream = 
